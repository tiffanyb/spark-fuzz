...
class Deadman:
    """Hold-to-run gate on SPACE. Releasing it freezes the replay.

    "Freeze" is republishing the LAST pose at the same rate, not going silent.
    Stopping the stream lets the motor controller time out, and a hoisted robot
    whose arms go limp drops them -- worse than holding still. So the gate stops
    the trace INDEX from advancing; the command keeps flowing.

    Held keys arrive as terminal auto-repeat, so "held" is really "a SPACE
    arrived within the last `timeout` seconds". macOS repeats after an initial
    delay of ~0.5 s, so expect one hesitation at the start of a hold. If your
    key repeat is off, use --hold-mode toggle instead, where SPACE flips
    run/pause and you do not have to hold anything.

    ESC or q aborts, which ramps stiffness down through the caller's finally.
    """

    def __init__(self, timeout=DEADMAN_TIMEOUT, enabled=True, toggle=False):
        self.timeout, self.enabled, self.toggle = timeout, enabled, toggle
        self.last = 0.0
        self.running = False
        self.abort = False
        self._fd = None
    ...
    def __enter__(self):
        if self.enabled:
            if not sys.stdin.isatty():
                raise SystemExit(
                    "the dead-man switch needs a terminal (stdin is not a tty).\n"
                    "Run it interactively, or pass --no-deadman to disable the "
                    "gate -- which means the replay runs to completion with no "
    ...
    def poll(self):
        """(may_advance, abort). Drains everything queued this tick."""
        if not self.enabled:
            return True, False
        ...
        return (time.monotonic() - self.last) < self.timeout, False
...
def report(rows, meta, joints, targets, stats, notes, viol, dt, stop, args):
    ...
    # A baseline trace has contact_step -1: there is no contact, so saying
    # "stopping before contact" would imply a hazard that is not there.
    c = t.get("contact_step", -1)
    if c is None or c < 0:
        tail = "  (no contact in this trace -- the legitimate task, safe to "
        tail += "replay in full)"
    elif args.through_contact:
        tail = ""
    else:
        tail = "  (stopping before contact)"
    print(f"  contact    step {c}   replaying rows 0..{stop-1}{tail}")
...
def live(rows, meta, joints, targets, stop, dt, args):
    ...
    dm = Deadman(args.deadman_timeout, not args.no_deadman, args.hold_mode == "toggle")
    try:
      with dm:
        ...
        if dm.enabled:
            print("\n  DEAD-MAN SWITCH ARMED"
                  + ("  --  SPACE toggles run/pause"
                     if dm.toggle else
                     "  --  HOLD SPACE to move, release to freeze"))
            print("  ESC or q aborts and ramps stiffness down.")
            print("  Nothing advances until you press it.\n")

        # The approach moves the arm to row 0, so it is gated too -- it is a
        # real motion, not a formality.
        print(f"  approaching row 0 over {args.approach}s (gated)")
        ...
        while k <= n:
            live, abort = dm.poll()
            if abort:
                print("\n  ABORT during approach"); return 1
            f = k / n
        ...
        print(f"  replaying {stop} steps ({stop*dt:.2f}s of trajectory)")
        i, frozen, was_live = 0, 0, None
        t_next = time.monotonic()
        while i < stop:
            live, abort = dm.poll()
            if abort:
                print(f"\n  ABORT at step {i}/{stop}")
                break
            if live != was_live:
                print(f"   [{'RUN ' if live else 'FREEZE'}] step {i}/{stop}"
            ...
            if live:
                ...
                # keep the stream alive at the pose already reached
                send(targets[max(i - 1, 0)], 1.0)
                frozen += 1
            t_next += dt
            slack = t_next - time.monotonic()
            if slack > 0:
                time.sleep(slack)
            if live and i % int(1 / dt) == 0:
                st_ = sub.Read(0)
                if st_ is not None:
                    j0 = joints[0]
                    print(f"   t={i*dt:5.2f}s  m{j0['real']} "
                          f"cmd={targets[min(i, stop-1)][j0['real']]:+.4f} "
                          f"meas={st_.motor_state[j0['real']].q:+.4f}")
        print(f"  replayed {i}/{stop} steps; frozen for "
              f"{frozen*dt:.1f}s of wall clock")
        print("  holding final pose")
        for _ in range(int(0.4 / dt)):
            send(targets[max(i - 1, 0)], 1.0); time.sleep(dt)
    finally:
...
def main(argv=None):
    ...
    p.add_argument("--no-deadman", action="store_true",
                   help="disable the hold-SPACE gate. The replay then runs to "
                        "completion with no keyboard stop -- only do this if "
                        "something else can cut power")
    p.add_argument("--hold-mode", choices=("hold", "toggle"), default="hold",
                   help="'hold' (default): move only while SPACE is held. "
                        "'toggle': SPACE flips run/pause, for terminals with "
                        "key repeat disabled")
    p.add_argument("--deadman-timeout", type=float, default=DEADMAN_TIMEOUT,
                   help=f"hold mode: freeze if no SPACE within this many "
                        f"seconds (default {DEADMAN_TIMEOUT})")
