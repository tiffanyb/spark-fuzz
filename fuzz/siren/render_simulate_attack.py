...
def pick_candidate(res, mode, selfcheck_glob):
    """Choose which discovered attack to render.

    `farthest` deliberately restricts to self-check-verified INSERTION goals:
    a MODIFICATION hit does its damage on the way TO the inserted goal, which
    is a different mechanism and would mislabel the legs in the video.
    """
    kinds = {}
    for f in sorted(glob.glob(selfcheck_glob)):
    ...
    for row in res["results"]:
        for h in row["hits"]:
            k = tuple(np.round(h["cand"], 9))
            e = uniq.setdefault(k, {"dist": h["dist_to_truth"],
                                    "clear": h["min_clearance"],
                                    "pickers": set()})
            e["pickers"].add(row["picker"])
            e["clear"] = min(e["clear"], h["min_clearance"])
    for k, e in uniq.items():
        e["kind"] = kinds.get(k)

    pool = [(k, e) for k, e in uniq.items() if e["kind"] == "INSERTION"] or \
           list(uniq.items())
    if mode == "farthest":
        k, e = max(pool, key=lambda kv: kv[1]["dist"])
    elif mode == "nearest":
        k, e = min(pool, key=lambda kv: kv[1]["dist"])
    elif mode == "deepest":
        k, e = min(pool, key=lambda kv: kv[1]["clear"])
    else:
        k, e = pool[0]
...
def main(argv=None):
    ...
    p.add_argument("--pick", default="farthest",
                   choices=["farthest", "nearest", "deepest"])
    ...
    cand, meta, n_uniq = pick_candidate(res, a.pick, a.selfcheck)
    ...
    print(f"  kind={meta['kind']}  found by {sorted(meta['pickers'])}  "
    ...
    out = a.out or str(Path(a.fuzz_result).parent /
                       f"attack_{res['target']}_{a.pick}.mp4")
    ...
    json.dump({"target": res["target"], "candidate": cand.tolist(),
               ...
               "kind": meta["kind"], "pickers": sorted(meta["pickers"]),
