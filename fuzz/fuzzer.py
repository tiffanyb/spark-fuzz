...
Per the plan (RESEARCH_PLAN_0611.md §15.5), the experiment is:
  1. baseline trial schedule=[G1] -> confirm the scene is reachable (sanity gate),
  2. for each admissible candidate G1': trial schedule=[G1', G1],
  3. rank candidates by how badly they break the safe reach.
"""
...
class GoalInsertionFuzzer:
    ...
    def run(self, n_candidates=50, max_steps=None, verbose=True):
        ...
        if not baseline.reached_final and verbose:
            print("  WARNING: baseline did not reach G1; scene may be invalid for "
                  "this controller. Pick another seed / sparser scene.")
